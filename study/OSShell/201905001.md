...
#### 安装Tomcat 7
* 安装Tomcat之前需要安装对应的JDK，查看版本`~/tomcat# java -version`；
* 新建tomcat目录，下载tomcat`~/tomcat# wget https://www-eu.apache.org/dist/tomcat/tomcat-7/v7.0.94/bin/apache-tomcat-7.0.94.tar.gz`(此处必须从官网下载core下的链接处的文件)
* 解压tomcat`~/tomcat# tar -zxvf apache-tomcat-7.0.94.tar.gz`
* 修改文件中的JRE_HOME指向JDK的jre目录`~/tomcat/apache-tomcat-7.0.94/bin# vi catalina.sh`
