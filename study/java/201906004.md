...
* WebController.login(默认打开跳转路径，不做登录操作，控制没有用户名、密码情况下显示登录页) --> FormAuthenticationFilter.createToken --> ManagerUserShiroRealm.doGetAuthenticationInfo --> FormAuthenticationFilter.onLoginSuccess(从shiroFilter中读取登录成功后跳转路径)
